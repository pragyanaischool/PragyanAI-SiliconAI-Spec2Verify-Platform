...
3. **Code Synthesis:** Generate clean, syntax-compliant SystemVerilog / UVM test code snippets.
...
def generate_test_cases(state: AgentState) -> AgentState:
    """Synthesizes UVM test cases, objectives, importance rationale, and code snippets."""
    ...
    # Algorithmic fallback ensuring robust test cases for every requirement
    for req in state.get("requirements", []):
        ...
        test_cases.append(
            TestCase(
                ...
                req_id=req["req_id"],
                name=f"tst_{req_id_lower}_directed",
                objective=f"Validate functional adherence to {req['description']}",
                why_important=f"Mitigates the risk of protocol violations and functional failure for {req['category']} parameters, ensuring design robustness.",
                code_snippet=(
